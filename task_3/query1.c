...
//QUERY that filters out students with 0 courses
...
int main(int argc, char *argv[]) {
    ...
    if (db){                            
		...
		printf("\033[0;32mQUERY1 [Students that chose 0 courses]:\033[0m\n");
        printf("Filter applied, %d students found\n", counterDemo); // how many passed the filters
		for (int i = 0; i < counterDemo; i++) {
			printf("%d %s %s year of study: %d load: %d laguages: %s \n", i + 1, students[indexArr[i]].name, students[indexArr[i]].surname, students[indexArr[i]].course, students[indexArr[i]].load, students[indexArr[i]].languages);
		}
		printf("\n");
        fclose(db); 
    } else {
