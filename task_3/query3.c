...
//QUERY that filters out students who names start with consonant
...
int main(int argc, char *argv[]) {
    ...
    if (db){                            
        ...
        for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            char	fl = s.name[0]; // takes the students name first letter
            if(!(fl == 'A' ||  fl == 'E' || fl == 'I' || fl == 'O' || fl == 'U')){ // *** first filter, conditions on the student
		...
		printf("\033[0;32mQUERY3 [Students who names start with Consonant]:\033[0m\n");
        printf("Filter applied, %d students found\n", counterDemo); // how many passed the filters
		for (int i = 0; i < counterDemo; i++) {
			printf("%d %s %s load: %d average: %f courses and grades ", i + 1, students[indexArr[i]].name, students[indexArr[i]].surname, students[indexArr[i]].load, students[indexArr[i]].average);
