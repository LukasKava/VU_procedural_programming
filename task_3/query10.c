...
//QUERY that filters out students who speak Lithuanian
            ...
            if(1){ // *** first filter, conditions on the student
                ...
                for (int i = 0; i < s.load; ++i){ // process each course taken by the student
					...
					if (strstr(s.languages, "Lithuanian") != NULL) {
						isLithuanianLanguage = true;
					}
                }
                printf("%s\n", s.languages);
                        
                if (isLithuanianLanguage == true) {//
                    ++counterDemo; // counting students
				}
            }
        }
		printf("\033[0;32mQUERY10 [Students who speak Lithuanian language]:\033[0m\n");
